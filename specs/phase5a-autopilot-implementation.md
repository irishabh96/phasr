...
**Stage B — QAS auto-approve for hands-off overnight (SHIPPED 2026-08-02, opt-in).** Delivered in the completion program's Phase 10: the `Agent(SpawnReviewer)` policy class behind the per-epic `require_human_approval` flag (migration 0017, **DEFAULTS ON** — a founder opts INTO letting an agent approve), `WorkspaceKind::Reviewer` rows (`reviews_subtask_id`, excluded from board/worklist/sidebar), the driver's reviewer spawn with one-live-reviewer dedup + resolve-time cleanup, the `approve`/`request-changes` CLI+IPC verbs gated by `GrantKind{Producer,Reviewer}`, server-side I6 re-verification (a red validate can never be approved past), and optimistic-concurrency `resolve_review` keyed on the exact `review.at_ms` the reviewer was spawned for (a human who acts first always wins). Ship remains structurally human — `SafeVerb` still has no `Ship` variant.

> DEFERRED-BY-DESIGN, still: the reviewer honest-status CARD (a QAS reviewer is backstage machinery today — its verdict lands on the ticket it reviews, and its own liveness is not surfaced) and the "take over from QAS" control. Both are presentation, not safety.
