...
Releases are built by GitHub Actions when a `v*` tag lands on the repo. Two parallel jobs produce a DMG for each Mac architecture and upload both to a draft Release. The maintainer writes notes and publishes.
...
4. **Watch the workflow.** Open Actions → Release. Two jobs run in parallel:
   - `macos-14` builds `aarch64-apple-darwin` → `Phasr_<version>_aarch64.dmg`
   - `macos-13` builds `x86_64-apple-darwin` → `Phasr_<version>_x64.dmg`

   Cold-cache build: ~10 min per arch. Warm: ~3 min.

5. **Inspect the draft Release.** When both jobs finish, open the draft on the Releases page. Confirm both DMGs are attached plus their `.app.tar.gz` siblings (Tauri uploads both for users who prefer extraction over DMG mount).
...
- `Phasr_<version>_x64.dmg` — Intel
- `Phasr.app.tar.gz` siblings — same .app, just zipped
